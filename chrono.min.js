...
var moment=require("moment"),Parser=require("../parser").Parser,ParsedResult=require("../../result").ParsedResult,PATTERN=new RegExp("(\\W|^)([0-9]{4})\\-([0-9]{1,2})\\-([0-9]{1,2})(?:T([0-9]{1,2}):([0-9]{1,2})(?::([0-9]{1,2})(?:\\.(\\d{1,4}))?)?(?:Z|([+-]\\d{2}):?(\\d{2})?)?)?(?=\\W|$)","i"),YEAR_NUMBER_GROUP=2,MONTH_NUMBER_GROUP=3,DATE_NUMBER_GROUP=4,HOUR_NUMBER_GROUP=5,MINUTE_NUMBER_GROUP=6,SECOND_NUMBER_GROUP=7,MILLISECOND_NUMBER_GROUP=8,TZD_HOUR_OFFSET_GROUP=9,TZD_MINUTE_OFFSET_GROUP=10;exports.Parser=function(){Parser.apply(this,arguments),this.pattern=function(){return PATTERN},this.extract=function(t,e,s,r){var t=s[0].substr(s[1].length),a=s.index+s[1].length,n=new ParsedResult({text:t,index:a,ref:e});if(n.start.assign("year",parseInt(s[YEAR_NUMBER_GROUP])),n.start.assign("month",parseInt(s[MONTH_NUMBER_GROUP])),n.start.assign("day",parseInt(s[DATE_NUMBER_GROUP])),moment(n.start.get("month"))>12||moment(n.start.get("month"))<1||moment(n.start.get("day"))>31||moment(n.start.get("day"))<1)return null;if(null!=s[HOUR_NUMBER_GROUP])if(n.start.assign("hour",parseInt(s[HOUR_NUMBER_GROUP])),n.start.assign("minute",parseInt(s[MINUTE_NUMBER_GROUP])),null!=s[SECOND_NUMBER_GROUP]&&n.start.assign("second",parseInt(s[SECOND_NUMBER_GROUP])),null!=s[MILLISECOND_NUMBER_GROUP]&&n.start.assign("millisecond",parseInt(s[MILLISECOND_NUMBER_GROUP])),null==s[TZD_HOUR_OFFSET_GROUP])n.start.assign("timezoneName",0);else{var R=0,_=parseInt(s[TZD_HOUR_OFFSET_GROUP]);null!=s[TZD_MINUTE_OFFSET_GROUP]&&(R=parseInt(s[TZD_MINUTE_OFFSET_GROUP]));var U=60*_;U<0?U-=R:U+=R,n.start.assign("timezoneName",U)}return n.tags.ENISOFormatParser=!0,n}};
...
var Refiner=require("./refiner").Refiner,TIMEZONE_NAME_PATTERN=new RegExp("^\\s*\\(?([A-Z]{2,10})\\)?(?=\\W|$)","i"),DEFAULT_TIMEZONE_ABBR_MAP={CDT:"US/Central",CST:"US/Central",CENTRAL:"US/Central",EAST:"US/Eastern",EASTERN:"US/Eastern",EDT:"US/Eastern",EST:"US/Eastern",MDT:"US/Mountain",MST:"US/Mountain",MOUNTAIN:"US/Mountain",PDT:"US/Pacific",PST:"US/Pacific",PT:"US/Pacific",PACIFIC:"US/Pacific"};exports.Refiner=function(e){Refiner.call(this,arguments),this.refine=function(e,i,n){var r=new Object(DEFAULT_TIMEZONE_ABBR_MAP);if(n.timezones)for(var t in n.timezones)r[t]=n.timezones[t];return i.forEach(function(i){if(i.tags.ENTimeExpressionParser||i.tags.ZHTimeExpressionParser||i.tags.FRTimeExpressionParser||i.tags.DETimeExpressionParser){var n=TIMEZONE_NAME_PATTERN.exec(e.substring(i.index+i.text.length));if(n){var t=n[1].toUpperCase();if(void 0===r[t])return;var a=r[t];i.start.isCertain("timezoneName")||i.start.assign("timezoneName",a),null==i.end||i.end.isCertain("timezoneName")||i.end.assign("timezoneName",a),i.text+=n[0],i.tags.ExtractTimezoneAbbrRefiner=!0}}}),i}};
},{"./refiner":64}],52:[function(require,module,exports){
var Refiner=require("./refiner").Refiner,TIMEZONE_OFFSET_PATTERN=new RegExp("^\\s*(GMT|UTC)?(\\+|\\-)(\\d{1,2}):?(\\d{2})","i"),TIMEZONE_OFFSET_SIGN_GROUP=2,TIMEZONE_OFFSET_HOUR_OFFSET_GROUP=3,TIMEZONE_OFFSET_MINUTE_OFFSET_GROUP=4;exports.Refiner=function(){Refiner.call(this),this.refine=function(e,E,n){return E.forEach(function(E){if(!E.start.isCertain("timezoneName")){var n=TIMEZONE_OFFSET_PATTERN.exec(e.substring(E.index+E.text.length));if(n){var O=parseInt(n[TIMEZONE_OFFSET_HOUR_OFFSET_GROUP]),T=parseInt(n[TIMEZONE_OFFSET_MINUTE_OFFSET_GROUP]),t=60*O+T;"-"===n[TIMEZONE_OFFSET_SIGN_GROUP]&&(t=-t),null!=E.end&&E.end.assign("timezoneName",t),E.start.assign("timezoneName",t),E.text+=n[0],E.tags.ExtractTimezoneOffsetRefiner=!0}}}),E}};
